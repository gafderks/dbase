...
class ListViewPage(EventViewPage):
    def switch_to_game_view(self):
        self.test.wait_for(
            lambda: self.test.browser.find_element(
                By.CLASS_NAME, "button-gameview"
            ).click()
        )
        # Wait for the page to switch, after switching the page should have a list view
        #  button to switch back.
        self.test.wait_for(
            lambda: self.test.browser.find_element(By.CLASS_NAME, "button-listview")
        )
    ...
    def verify_booking_attributes(
        ...
        game_name_elem = booking.find_element(By.CLASS_NAME, "booking-game")
        self.test.wait_for(
        ...
        daypart_elem = booking.find_element(
            By.XPATH, "." + "/.." * 6
        )  # 6 levels higher
